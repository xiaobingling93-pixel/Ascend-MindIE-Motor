...
class PathCheckBase(object):
    ...
    def check_exists(cls, path: str):
        if not os.path.exists(path):
            error_message = f"[OM Adapter] The path {path} is not exists"
    ...
    def check_soft_link(cls, path: str):
        if os.path.islink(os.path.normpath(path)):
            error_msg = f"[OM Adapter] The path {path} is a soft link"
    ...
    def check_owner_group(cls, path: str, is_support_root: bool = True):
        ...
        if flag:
            return True
        error_msg = f"[OM Adapter] Check the path {path} owner and group failed"
        cls.log_error(error_msg)
        return False
    ...
    def check_path_mode(cls, mode: int, path: str):
        ...
        if cur_mode != mode:
            error_message = f"[OM Adapter] Check the {path} %s mode failed"
            cls.log_error(error_message)
            return False
        return True
    ...
    def check_name_valid(cls, path: str):
        if not path:
            error_msg = f"[OM Adapter] The path is empty: {path}"
            cls.log_error(error_msg)
            return False
        if len(path) > 2048:
            error_msg = f"[OM Adapter] The length of path exceeds 2048 characters: {path}"
            cls.log_error(error_msg)
            return False
        traversal_patterns = ["../", "..\\", ".."]
        if any(pattern in path for pattern in traversal_patterns):
            error_msg = f"[OM Adapter] Path contains traversal sequences: {path}"
            cls.log_error(error_msg)
            return False
        if re.search(r'[^0-9a-zA-Z_./-]', path):
            error_msg = f"[OM Adapter] The path contains special characters: {path}"
            cls.log_error(error_msg)
            return False
        return True
    ...
    def check_file_size(cls, path: str, max_file_size: int = 1024 * 1024 * 1024):
        try:
            ...
            cls.log_error("[OM Adapter] The path %s is not exists" % path)
            return False
        if file_size > max_file_size:
            cls.log_error(f"[OM Adapter] Invalid file size, should be no more than {max_file_size} but got {file_size}")
            return False
        return True
