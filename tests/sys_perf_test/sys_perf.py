...
if __name__ == "__main__":
    ...
    mies_install_path = environment_config.get(
        "mies_install_path", "/usr/local/Ascend/mindie/latest/mindie-service"
    )

    # start container
    command_helper_instance.exec_command(
        server_id, f"docker start {container_name}", wait_time=5
    )
    command_helper_instance.exec_command(
        server_id, f"docker exec -it {container_name} bash", wait_time=5
    )
    ...
    if args.compile or args.deployment:
        try:
            ...
            print_to_screen(
                "Deployment failed, please check the compile result and try again"
            )
    ...
    for test_case in test_cases:
        ...
        # kill all service
        command_helper_instance.exec_command(
            server_id, f"source {parent_dir}/kill_all_service.sh", wait_time=5
        )
        command_helper_instance.exec_command(
            client_id, f"source {parent_dir}/kill_all_service.sh", wait_time=5
        )

        # start service
        command_helper_instance.exec_command(
            server_id,
            f"export MIES_CONFIG_JSON_PATH={test_case['config_path']}",
            wait_time=1,
        )
        command_helper_instance.exec_command(
            server_id,
    ...
    save_results(
        results_of_performance_test, f"{args.outputdir}/output_preformance.csv"
    )
