...
Prefill&Decode分离服务部署（简称：PD分离服务部署），PD分离服务部署又分为单机PD分离服务部署和多机PD分离服务部署，是将Prefill（预填充）和Decode（解码）这两个推理阶段分开处理的技术，适用于对时延有严格要求的场景。PD分离服务部署可以提高NPU的利用率，尤其是大语言模型，将Prefill实例和Decode实例分开部署，减少Prefill阶段和Decode阶段分时复用在时延上造成的互相干扰，实现同时延下吞吐量提升。PD分离工作原理如[图1](#fig7842711175715)所示。
...
- 资源利用优化：由于Prefill阶段计算密集，而Decode阶段计算较为稀疏，将这两个阶段分离可以更有效地利用NPU的计算资源。
...
|类型|型号|内存|
|--|--|--|
|服务器|Atlas 800I A2 推理服务器|<li>32GB</li><li>64GB</li>|
...
4. 配置Server服务启动的config.json配置文件（文件所在目录： *{mindie-llm安装目录}*/conf/config.json），PD分离服务部署模式需要配置的参数如[表2 config.json关键配置参数解释](#table472745002618)所示，具体参数解释请参见《MindIE LLM开发指南》中[配置参数说明（服务化）](https://gitcode.com/Ascend/MindIE-LLM/blob/master/docs/zh/user_guide/user_manual/service_parameter_configuration.md)章节。
    ...
    **表 2** config.json关键配置参数解释<a id="table472745002618"></a>
    ...
    在单机PD分离部署场景下，多个Server进程将运行于同一Pod中，每个Server进程需对应独立的配置文件。为简化配置流程，可使用mindie_service_single_container.yaml文件（文件所在目录： *{mindie-motor安装目录}*/src/example/deploy_scripts/deployment）中的MINDIE_MS_GEN_SERVER_PORT环境变量进行管理。该环境变量支持两种配置模式：
...
6. 配置kubernetes Deployment，主要配置参数如下所示。
    ...
    >- 用户在使用kubectl部署Deployment时，可修改deployment的配置yaml文件，请避免使用危险配置，确保使用安全镜像（非root权限用户），配置安全的Pod上下文。
...
- Atlas 800I A2 推理服务器需要配置卡IP并安装Ascend Operator组件。
- 当前部署脚本不支持NPU故障重调度场景。
- 异构场景中的P实例需部署在Atlas 800I A2 推理服务器（32G）设备上，D实例需部署在Atlas 800I A2 推理服务器（64G）设备上。
