...
from om_adapter.common.util import PathCheckBase


MODULE_NAME = 'mindie-om-adapter'
...
logger_screen = logging.getLogger("adapter_screen")
...
def print_value_warn(value_type, env_value, default_value, support_choices=None):
    msg = f"[WARNING] Invalid value of om_adapter in {value_type}: {env_value}. "
    if support_choices:
...
class Log(metaclass=Singleton):
    ...
    def __init__(self, logger=None):
        self._logger = logging.getLogger('om-adapter')
        # 从环境变量获取日志路径与级别
    ...
    def get_log_rotate_from_env(cls, env_values: dict, parse_type: str) -> dict:
        ...
        参数 fs fileSize: 整数 默认20 单位MB
             r fileCountPerProcess: 整数 个数, 每个进程fc个日志
        """
        # 日志文件默认最大20MB，每个进程默认一个日志文件
        option_dict = {
