...
>   - 最大4万条并发请求数配置为"48Gi"。
>   - 最大9万条并发请求数配置为"108Gi"。
> **Coordinator占用的最大内存可以通过公式计算：内存规格 = boby_limit * max_requests * 120%**
>   - boby_limit：单条请求消息体的字节数上限，单位为MB，可在ms_coordinator.json配置文件中配置与查看，具体参数解释详情参见[ms_coordinator.json启动配置文件参数说明](../cluster_management_component/coordinator.md#section1759442933513)。
...
>   - 最大9万条并发请求数配置为"108Gi"。
> **Coordinator占用的最大内存可以通过公式计算：内存规格 = boby_limit * max_requests * 120%**
>   - boby_limit：单条请求消息体的字节数上限，单位为MB，可在ms_coordinator.json配置文件中配置与查看，具体参数解释详情参见xxx。
