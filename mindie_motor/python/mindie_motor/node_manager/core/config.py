...
class GeneralConfig(metaclass=_SingletonMeta):
    ...
    def _check_server_config_valid(config):
        ...
        if "distDPServerEnabled" not in server_config or not isinstance(
            server_config["distDPServerEnabled"], bool
        ):
            raise KeyError(
                "distDPServerEnabled is Missing or Invalid in Server Engine Config File"
            )
    ...
    def _load_server_engine_ip():
        ...
        if pod_ip is None:
            raise KeyError(
                "[Config] Please Check Environment Variable Configuration of POD_IP"
            )
    ...
    def _load_config_path(cls):
        ...
        if mindie_install_path is None:
            raise KeyError(
                "[Config] Please Check Environment Variable Configuration of MIES_INSTALL_PATH"
            )
        mindie_install_path = os.path.realpath(mindie_install_path)
        config_path = os.path.join(mindie_install_path, "conf")
        if not os.path.exists(config_path):
            raise FileNotFoundError(
                f"[Config] Cannot find Config Directory: {config_path}."
            )
        return config_path
    ...
    def _update_info(self):
        ...
        self.http_server_config = JsonUtil.read_json_file(
            os.path.join(self.config_path, "node_manager.json")
        )
        self.server_engine_port_list = (
            self._load_server_engine_ports()
        )  # ['10000','10001',...]
    ...
    def _is_master_node(self):
        ...
        if ranktable_path is None:
            raise KeyError(
                "Please Check Environment Variable Configuration of RANK_TABLE_FILE"
            )
        ...
        if not isinstance(server_list, list) or not server_list:
            raise KeyError(
                "Invalid ranktable.json file: server_list is missing or invalid"
            )
    ...
    def _load_controller_port(self):
        if "controller_alarm_port" not in self.http_server_config:
            raise KeyError("[Config] Controller Server Port is Missing in Config File")
