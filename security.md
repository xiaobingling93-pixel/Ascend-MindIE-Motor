...
- 建议用户对个人隐私数据、商业资产、业务开发相关的各类包含敏感内容的文件做好访问权限控制。例如本项目中安装目录权限管控、数据文件权限管控，设定的权限可参考[A-文件（夹）各场景权限管控推荐最大值](#section0001)。
...
- 本项目默认的Gloo、DataDist和HCCL通信暂不支持TLS认证功能，如有需要，可参考[B-集合通信加固](#section0002)。
...
### A-文件（夹）各场景权限管控推荐最大值<a id="section0001"></a>
