...
build_grpc() {
    ...
    if [ ! -d "$src_grpc_dir"/third_party/re2 ]; then
        ...
        bash "$MINDIE_SERVICE_SRC_DIR"/ci/patch_grpc_and_dependency.sh
    fi
...
build_mockcpp() {
    ...
    if [ -f "$MINDIE_SERVICE_SRC_DIR"/tests/server/update.patch ]; then
        cd ${src_mockcpp_dir}
        patch -p1 -f < "$MINDIE_SERVICE_SRC_DIR"/tests/server/update.patch || true
    fi
