...
def wait_hccl_ranktable_finish(configmap_name, namespace="mindie"):
    hccl_ranktable_str = output_from_kubectl(
        "kubectl get configmap %s -n %s -o jsonpath='{.data}'"
        ...
        if deployment_label_json_obj["ring-controller.atlas"] == "ascend-310p":
            ...
            while True:
                pod_ips = output_from_kubectl(
                    ...
                    "app=mindie-server -o jsonpath='{.items[*].status.podIP}' -n "
                    + namespace
                ).split(" ")
                if len(pod_ips) != int(replicas) or not pod_ips or not pod_ips[0]:
                    logging.info(
                        "Getting pod ips with label ring-controller.atlas=ascend-310p"
                    )
                    time.sleep(2)
                    continue
                else:
            ...
            for i in range(int(replicas)):
                hccl_rank_table[SERVER_LIST_KEY].append(
                    {"server_id": pod_ips[i], "container_ip": pod_ips[i]}
                )
            patch_data = f'{{"data": {{"hccl.json": {json.dumps(json.dumps(hccl_rank_table, indent=4))}}}}}'
            prefill_str = (
                "kubectl patch configmap rings-config-mindie-server -n "
                + namespace
                + " --type merge -p "
            )
...
def add_mindie_server_group(
    heter, deployment_name, group_id, server_num=1, namespace="mindie"
):
    ...
    while True:
        ...
        if elapsed_time > TIMEOUT_SECONDS:
            return None
        finish, hccl_ranktable, hardware_type = wait_hccl_ranktable_finish(
...
def add_mindie_ms_group(
    namespace: str, module_name: str, backup_enable: bool = False
) -> dict:
    while True:
        pod_ip = output_from_kubectl(
            "kubectl get pods -l app=mindie-ms-"
            + module_name
            + " -n "
            + namespace
...
def ac_job_state_log(namespace):
    ...
    for line in ac_job_state_data:
        ...
        ac_job_state_log_lines.append(
            f"Namespace: {namespace}, Instance Name: {instance_name}, State: {state};"
        )
...
def generate_global_ranktable(
    heter=False, p_node_num=1, d_node_num=1, is_multi_node=False, ext=None
):
    ...
    if is_multi_node:
        for i in range(p_node_num):
            server_group = add_mindie_server_group(
                heter,
                RINGS_CONFIG_P + str(i),
                group_start_num,
                ext[P_DEPLOY_SERVER],
                namespace,
            )
        ...
        if ext is not None and ext["is_acjob"] == "true":
            group_map = build_group_map(
                d_node_num, ext, group_start_num, heter, p_node_num
            )
            group_map["server_count"] = str(len(group_map[SERVER_LIST]))
            global_rank_table[SERVER_GROUP_LIST_KEY].append(group_map)
        else:
            server_group = add_mindie_server_group(
                heter, "rings-config-mindie-server", group_start_num, -1, namespace
            )
            if server_group is not None:
                global_rank_table[SERVER_GROUP_LIST_KEY].append(server_group)
    controller_backup_enable = (
        ext.get("controller_backup_enable", False) if ext else False
    )
    ...
    prefill_str = (
        "kubectl patch configmap global-ranktable -n " + namespace + " --type merge -p "
    )
    ret = output_from_kubectl(f"{prefill_str} '{patch_data}'", False)
    if ret is None:
        logging.info("return is None")
...
def build_group_map(d_node_num, ext, group_start_num, heter, p_node_num):
    ...
    for i in range(p_node_num):
        if not group_map:
            group_map = add_mindie_server_group(
                heter, RINGS_CONFIG_P + str(i), group_start_num, ext[P_DEPLOY_SERVER]
            )
        else:
            server_list = add_mindie_server_group(
                heter, RINGS_CONFIG_P + str(i), group_start_num, ext[P_DEPLOY_SERVER]
            )[SERVER_LIST]
    ...
    for i in range(d_node_num):
        if not group_map:
            group_map = add_mindie_server_group(
                heter, RINGS_CONFIG_D + str(i), group_start_num, ext[D_DEPLOY_SERVER]
            )
        else:
            server_list = add_mindie_server_group(
                heter, RINGS_CONFIG_D + str(i), group_start_num, ext[D_DEPLOY_SERVER]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Enable or disable heterogeneous deployment",
    )
    parser.add_argument(
        "--is_multi_node",
        type=bool,
        default=False,
        help="Enable or disable multi node feature",
    )
