...
from om_adapter.backends import select_backend
from om_adapter.config import ConfigUtil
from om_adapter.common.logging import Log
from om_adapter.common.cert_util import AdapterCertUtil
...
class BaseMonitor(ABC):
    def __init__(self, backend_name: str, identity: str, *args, **kwargs):
        ...
        monitor_config = ConfigUtil.get_config("ccae")
        ...
        self.tls_config = ConfigUtil.get_config("tls_config")
        if self.tls_config and self.tls_config.get("ccae_tls_enable"):
            self.logger.info("Sending requests with ssl!")
            ssl_context = create_default_context(Purpose.SERVER_AUTH)
            ccae_tls_items = self.tls_config.get("ccae_tls_items")
            ssl_context.load_verify_locations(cafile=ccae_tls_items["ca_cert"])
            password = AdapterCertUtil.validate_cert_and_decrypt_password(ccae_tls_items)
            ssl_context.load_cert_chain(
                certfile=ccae_tls_items["tls_cert"],
                keyfile=ccae_tls_items["tls_key"],
                password=password
            )
            ...
            del password
            self.http_pool_manager = urllib3.PoolManager(ssl_context=ssl_context, assert_hostname=False, timeout=5)
        else:
            self.logger.info("Sending requests without ssl!")
            self.http_pool_manager = urllib3.PoolManager(cert_reqs="CERT_NONE", timeout=5)
        self.logger.info(f"Detect monitor config successfully! monitor_ip = {self.monitor_ip}, "
                         f"monitor_http_port = {monitor_http_port}")
        self.url_prefix = f"https://{self.monitor_ip}:{monitor_http_port}"
    ...
    def init_producer(self):
        ...
        if not self.tls_config or not self.tls_config.get("ccae_tls_enable"):
        ...
        kafka_config.update({
            ...
            "ssl.ca.location": ccae_tls_items["ca_cert"],
            "ssl.certificate.location": ccae_tls_items["tls_cert"],
            "ssl.key.location": ccae_tls_items["tls_key"],
            "ssl.key.password": password
        })
        if ccae_tls_items["tls_crl"]:
            kafka_config["ssl.crl.location"] = ccae_tls_items["tls_crl"]
        self.producer = KafkaProducer(kafka_config)
        password_len = len(password)
        password_offset = sys.getsizeof(password) - password_len - 1
        ctypes.memset(id(password) + password_offset, 0, password_len)
    ...
    def run(self):
        ...
        while not self.heart_beat_flag:
            time.sleep(1)
        ...
        if self.identity == "Coordinator":
            self.logger.info("Inventory shouldn't be reported when identity is `Coordinator`.")
            return
