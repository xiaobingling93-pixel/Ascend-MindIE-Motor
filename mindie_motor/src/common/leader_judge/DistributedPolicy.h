...
constexpr int ETCD_CONNECT_MAX_RETRY = 300;     // etcd连接最大重试次数
constexpr int ETCD_CONNECT_RETRY_INTERVAL = 5;  // etcd连接重试间隔（秒）
...
struct EtcdTimeInfo {
    ...
    // 可定制化EtcdTimeInfo
    EtcdTimeInfo(int staticEtcdWatchGap, int staticRpcTimeout,
                 int staticLeaseTtl)
...
class DistributedLockPolicy {
   public:
    ...
    virtual bool GetWithRevision(const std::string& key,
                                 std::string& value) = 0;
};

class EtcdDistributedLock : public DistributedLockPolicy {
   public:
    EtcdDistributedLock(const std::string& etcdAddr, const std::string& lockKey,
                        const std::string& clientId, TlsItems& tlsConfig,
    ...
    void SetKvStub(std::shared_ptr<etcdserverpb::KV::StubInterface> stub) {
        kv_stub_ = stub;
    }
    void SetLeaseStub(
        std::shared_ptr<etcdserverpb::Lease::StubInterface> stub) {
        lease_stub_ = stub;
    }
...
class LeaseKeepAliveSession {
   public:
    ...
    std::unique_ptr<
        grpc::ClientReaderWriterInterface<etcdserverpb::LeaseKeepAliveRequest,
                                          etcdserverpb::LeaseKeepAliveResponse>>
        stream;

    explicit LeaseKeepAliveSession(etcdserverpb::Lease::StubInterface& stub,
                                   int staticLeaseTtl) {
        context = std::make_unique<grpc::ClientContext>();
        if (context != nullptr) {
            context->set_deadline(std::chrono::system_clock::now() +
                                  std::chrono::seconds(staticLeaseTtl));
