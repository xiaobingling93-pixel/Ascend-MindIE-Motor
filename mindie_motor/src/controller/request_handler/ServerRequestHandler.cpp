...
nlohmann::json ServerRequestHandler::BuildLocalNodeInfo(const NodeInfo &node) const
{
    nlohmann::json local;
    local["device"] = nlohmann::json::array();
    local["server_ip"] = node.ip;
...
nlohmann::json ServerRequestHandler::BuildPeerNodeInfos(std::vector<std::unique_ptr<NodeInfo>> &nodeVec,
    const NodeInfo &node) const
{
...
{
    nlohmann::json body;
    body["local"] = BuildLocalNodeInfo(node);
    body["peers"] = BuildPeerNodeInfos(nodeVec, node);
    return body.dump();
}
